...
n_objectives = 1 
n_initial=20 #number of initial experiments performed by random or latin hyper cube before gausian process is used
...
def xi_tweak(total_iterations,n_init,iteration):
    ''' This function is used to change the xi parameter each iteration. 
    The loop starts changing the parameter after n_initial iterations and 
    the function is made such that the parameter starts with a value of 1 and decays
    in a logaritmic fashion to close to zero over the iterations (total-initial).

    Parameters
    ----------
    total_iterations : int
        total ammount of iterations performed in the full experiment
    n_init : int
        same parameter fed to the optimizer instance that specifies when the bayesian optimizaiton starts
    iteration : int
        the current iteration
    
    Returns
    -------
    xi parameter as a float to use in the optimizer

    '''
    from math import log10
    iter_with_tweak=total_iterations-n_init
    xi=1-log10(1+9/(iter_with_tweak+1)*(iteration-n_init))
    return xi
...
for i in range(100000):
    ...
    if tecan_running == '"running"':
        ...
        if iteration == 0: #in the first iteration no data needs to be extracted
            ...
            if iteration >= n_initial: #when the iteration surpases n_initial the xi parameter is updated using xi_tweaker
                xi_temp = xi_tweak(48, n_initial, iteration)
                acq_func_kwargs = {"xi": xi_temp}
                opt.acq_func_kwargs = acq_func_kwargs # this sets the acq_kwarg to the new xi
                opt.update_next() #important for updating the optimizer instance

                print(f"Optimizer running with {opt.acq_func_kwargs} kwargs")

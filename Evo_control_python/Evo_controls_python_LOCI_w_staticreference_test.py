...
log_file = 'C:\\temp\\compressedLog_20210901.csv'
batch_size = 8
comparator_delta = 192
...
def make_recipe_file(link_to_recipe_folder, iteration, recipe):
    ...
    if batch_size == 1:
        ...
        with open(recipe_file_first_add_falcon, mode='w', newline='') as file:
            ...
            writer.writerow(['FALCON1', '1', '384WellMP', str(iteration), str(next_x[0])])  # ACC
            writer.writerow(
                ['FALCON1', '2', '384WellMP', str(iteration), str((15 - next_x[0]) * next_x[1])])  # Bio
            writer.writerow(['FALCON1', '3', '384WellMP', str(iteration), str(20 - (
                        next_x[0] + ((15 - next_x[0]) * next_x[1]) + next_x[2]))])  # Vol(buffer), as a remainder of the total volume to 20 uL
            writer.writerow(
                ['FALCON1', '1', '384WellMP', str(iteration + comparator_delta), str(next_x[0])])  # ACC
            writer.writerow(['FALCON1', '2', '384WellMP', str(iteration + comparator_delta),
                             str((15 - next_x[0]) * next_x[1])])  # Bio
            writer.writerow(['FALCON1', '3', '384WellMP', str(iteration + comparator_delta), str(20 - (
                        next_x[0] + ((15 - next_x[0]) * next_x[1])))])  # Vol(buffer), as a remainder of the total volume to 20 uL
        ...
        with open(recipe_file_first_add_Amber, mode='w', newline='') as file:
            writer = csv.writer(file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
            writer.writerow(['AMBER1', '1', '384WellMP', str(iteration), str(next_x[2])])  # Sample
        ...
        with open(recipe_file_second_add_falcon, mode='w', newline='') as file:
            writer = csv.writer(file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
            writer.writerow(['FALCON1', '4', '384WellMP', str(iteration), str(next_x[3])])  # Donor
            writer.writerow(['FALCON1', '5', '384WellMP', str(iteration), str(30 - next_x[3])])
            writer.writerow(['FALCON1', '4', '384WellMP', str(iteration + comparator_delta),
                             str(next_x[3])])  # Donor
            writer.writerow(
                ['FALCON1', '5', '384WellMP', str(iteration + comparator_delta), str(30 - next_x[3])])
...
def make_log(link_to_log_file, iteration, latest_exp, list_of_transformed_meassurements):
    with open(link_to_log_file, mode='a', newline='') as file:
        ...
        if iteration == 1:
            ...
            writer.writerow(latest_exp.append(list_of_transformed_meassurements))
        elif iteration > 1 and batch_size > 1:
            for i in range(batch_size):
...
def make_optimizer():
    space = [(1., 15.),(0.01, 1.),(1., 5.),(1., 30.)]
    lhs = True
    n_objectives = 1
    kernel_intern = 1 ** 2 * Matern(length_scale=[1, 1, 1, 1], length_scale_bounds=[(0.05, 10)], nu=2.5)
...
for i in range(100000):
    ...
    if tecan_running == '"running"':
        ...
        if iteration == 0:
            ...
            time.sleep(5)
            latest_file = r'C:\Users\Public\Documents\Tecan\SparkControl\Workspaces\LOCI_20220216_184348\Export\xlsx\LOCI_20220216_184804.xlsx'
